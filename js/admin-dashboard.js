...
// Mappings (compressed from old logic)
const TITLE_PARENT = {
...
function notify(msg,type='info'){ enqueueToast(msg,type); }
const TOAST_LIMIT = 5;
let toastQueue = [];
let activeToasts = [];
function enqueueToast(message,type='info',options={}){ const id=Date.now()+Math.random(); toastQueue.push({id,message,type,options}); processToastQueue(); }
function processToastQueue(){ const stack=document.getElementById('toast-stack'); if(!stack) return; while(activeToasts.length<TOAST_LIMIT && toastQueue.length){ renderToast(toastQueue.shift(), stack); } }
